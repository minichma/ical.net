...
internal class CollectionHelpersTests
{
    ...
    [Test]
    public void TestOrderedNestedMergeMany_HandlesEmptyAndNullInnerSequences()
    {
        IEnumerable<IEnumerable<int>> withNullableInner =
        [
            null,
            [],
            [1, 3],
        ...
        var result = withNullableInner
            .Select(x => x ?? Array.Empty<int>())
            .OrderedNestedMergeMany(Comparer<int>.Default)
            .ToList();
